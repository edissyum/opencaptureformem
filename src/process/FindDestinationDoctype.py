...
# Inférence
# =========================
def run_inference_doctype_entity(
    model_path: str,
    image
):
...
def run_inference_doctype_entity_remote(config, image):
    timeout = int(config.get("_remote_timeout", 300))

    with open(image.filename, "rb") as img_file:
        img_data = img_file.read()

    url = config.get("doctype_entity_remote_url")
    if not url:
        return False, "Remote destination inference not configured"
    password = config.get('doctype_entity_remote_password')
    ...
    if password and token:
        ...
        # HTTPS login method
        files_ok, files_error = get_runtime_files_state(config, "doctype_entity")
        if not files_ok:
            return False, files_error

        ca_cert = get_ca_crt_path(config, "doctype_entity")

        try:
            headers = build_jwt_headers(config, "doctype_entity", content_type="image/jpeg")
            ...
            if response.status_code == 401:
                clear_jwt_cache(config, "doctype_entity")
                headers = build_jwt_headers(config, "doctype_entity", content_type="image/jpeg", force_refresh=True)
                response = requests.post(
                    url,
                    headers=headers,
                    data=img_data,
