...
def run_inference_destination_remote(config, image):
    ...
    password = config.get('doctype_remote_password')
    token = config.get('doctype_remote_token')
    if not url or not password or not token:
    ...
    try:
        ...
        response = requests.post(
            config.get('doctype_remote_url'),
            headers={
                'Authorization': 'Bearer ' + config.get('doctype_remote_token'),
