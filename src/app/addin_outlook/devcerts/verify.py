...
from . import defaults  # Assuming defaults.py is available

def get_verify_command(return_invalid_certificate=False):
    if sys.platform == "win32":
        script = Path(__file__).parent / "../scripts/verify.ps1"
        default_command = f"powershell -ExecutionPolicy Bypass -File \"{script}\" -CaCertificateName \"{defaults.certificate_name}\" -CaCertificatePath \"{defaults.ca_certificate_path}\" -LocalhostCertificatePath \"{devcerts.defaults.localhost_certificate_path}\""
...
def verify_certificates(certificate_path=defaults.localhost_certificate_path, key_path=defaults.localhost_key_path):
